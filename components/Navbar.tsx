...
const PRODUCTS = [
  { href: '/loans?tab=personal', label: 'Personal Loan', icon: '👤', desc: 'From 15k salary · 10.25% p.a.' },
  { href: '/loans?tab=home', label: 'Home Loan', icon: '🏠', desc: 'Salaried, Self-employed, NRI' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
                  ...
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-2 overflow-hidden">
                    {PRODUCTS.map((p) => (
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-semibold text-navy group-hover:text-navy-dark">{p.label}</p>
          ...
          <div className="hidden md:flex items-center">
            <Link
              href="/apply"
              className="bg-navy text-white px-5 py-2 rounded-lg font-semibold text-sm hover:bg-navy-light transition-colors shadow-sm"
            >
              Get Loan
            </Link>
          </div>
          ...
          <div className="md:hidden pb-4 border-t border-gray-100 mt-1 pt-3 space-y-1">
            ...
            {PRODUCTS.map((p) => (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-gray-700 hover:bg-navy-50 hover:text-navy transition-colors"
              >
                <span>{p.icon}</span>
